import time


class Person:
    def __init__(self, name, salary, item=None):
        self.net_worth = 0
        self.name = name
        self.hourly_salary = float(salary)
        self.stuff = list()
        self.stuff = item if item else []

    def introduce_self(self):
        print(f'My name is {self.name}')

    def check_net_worth(self):
        print(self.net_worth)
        return self.net_worth

    def buy_something(self, thing):
        item = thing['item']
        price = thing['price']
        if self.net_worth >= price:
            self.stuff.append(item)
            self.net_worth -= price
        else:
            print("Not enough funds to buy this item.")

    def sell_something(self, thing):
        item = thing['item']
        price = thing['price']
        if item in self.stuff:
            self.stuff.remove(item)
            self.net_worth += price
        else:
            print("Item not found in possessions.")

    def do_work(self, hours):
        for h in range(int(hours)):
            time.sleep(1)
            self.net_worth += self.hourly_salary

    def show_possessions(self):
        print(f"{self.name}'s possessions: {self.stuff}")


item = ["apple", "banana", "peach"]
person1 = Person("Maria", 55, item)
person2 = Person("Steve", 50)

person1.introduce_self()
person1.do_work(4)

print('person 1: ')
person1.check_net_worth()
person1.show_possessions()

person2.introduce_self()
person2.do_work(8)

print('person 2: ')
person2.check_net_worth()

person1.buy_something({"item": 'apple', 'price': 250})
print("Maria's net worth:", person1.net_worth)

person1.sell_something({"item": 'toy car', 'price': 1.25})
print("Maria's stuff:", person1.stuff)
print("Maria's net worth:", person1.net_worth)
print("Maria's net worth after selling:", person1.net_worth)


